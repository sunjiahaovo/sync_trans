...
void talker::registerPubSub(){
  ...
  vins_sub  = new message_filters::Subscriber<nav_msgs::Odometry>(nh, "/vins_estimator/odometry", 10);
  optitrack_sub  = new message_filters::Subscriber<geometry_msgs::PoseStamped>(nh, "/vrpn_client_node/jiahao_sun/pose", 10);

  sync = new  message_filters::Synchronizer<mySyncPolicy>(mySyncPolicy(100), *vins_sub, *optitrack_sub);
